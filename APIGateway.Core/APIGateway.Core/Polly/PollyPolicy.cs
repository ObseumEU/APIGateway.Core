...
namespace APIGateway.Core.Polly
{
    ...
    public class PollyPolicy : IPollyPolicy
    {
        ...
        public AsyncRetryPolicy AutoRetryExponencial(int count = 13, string logRetryMessage = null)
        {
            return Policy
                .Handle<Exception>()
                .WaitAndRetryAsync(count, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)), (exception, retryCount, context) => _log.LogError($"try: {retryCount}, Message: {logRetryMessage} Exception: {exception.Message} InnerException:{exception.InnerException?.InnerException?.Message}"));
        }
    }
}
